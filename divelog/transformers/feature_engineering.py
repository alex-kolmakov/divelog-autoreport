...
def calculate_ascend_speed(data):
    ...
    data["ascend_speed"] = (
        data["ascend_speed"] * -1
    )  # Because positife depth diff means we are gaining depth
    data["ascend_speed"] = (
...
def feature_extract(data, *args, **kwargs):
    ...
    features["depth_variability"].fillna(
        features["depth_variability"].mean(), inplace=True
    )
    features["temp_variability"].fillna(
        features["temp_variability"].mean(), inplace=True
    )
    features["pressure_variability"].fillna(
        features["pressure_variability"].mean(), inplace=True
    )
...
def test_output(output, *args) -> None:
    ...
    assert output[
        output.isna().any(axis=1)
    ].empty, "There are NaN values in the dataframe"
